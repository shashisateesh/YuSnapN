...
namespace YuniquePLM.Mobility.iOS.UIViews.ApplicationLayer

{
	...
	public class ELCImagePickerViewController : UINavigationController
	{
		...
		private static Lazy<UIImage> mDefaultOverlayImage = new Lazy<UIImage>(() => UIImage.FromBundle("Overlay.png"));
		...
		bool ShouldSelectAsset(ALAsset asset, int previousCount)
		{
			...
			if (!shouldSelect)
			{
				var first =l10n.t("Only");
				var second =l10n.t("photos please!");

				string title = String.Format("{0} {1} {2}", first, MaximumImagesCount, second);

				var third =l10n.t("You can only send");
				var fourth =l10n.t("photos at a time.");

				string message = String.Format("{0} {1} {2}", third, MaximumImagesCount, fourth);
				var alert = new UIAlertView(title, message, null, null,l10n.t("Okay"));
				alert.Show();
			}
			return shouldSelect;
		}
		...
		public class ELCAlbumPickerController : UITableViewController
		{
			...
			public override void ViewDidLoad()
			{
				...
				NavigationItem.Title =l10n.t("Loading...");
			...
			void ReloadTableView()
			{
				...
				NavigationItem.Title =l10n.t("Select an Album");
			}
			...
			public override UITableViewCell GetCell(UITableView tableView, NSIndexPath indexPath)
			{
				...
				{
					Console.WriteLine("{0} {1}",l10n.t("Failed to set thumbnail"), e);
				}
		...
		class ELCAssetTablePicker : UITableViewController
		{
			...
			void PreparePhotos()
			{
				...
				_Dispatcher.BeginInvokeOnMainThread(() =>
				{
					...
					NavigationItem.Title = SingleSelection ?l10n.t("Pick Photo") :l10n.t("Pick Photos");
				});
			}
				...
				public void SetAssets(List<ELCAsset> assets, int columns)
				{
					...
					for (int i = 0; i < RowAssets.Count; i++)
					{
						...
						try
						{
							...
							if (asset.Asset != null && asset.Asset.Thumbnail != null)
							{
								...
								if (i < ImageViewArray.Count)
								{
									//var imageView = ImageViewArray[i];
									//imageView.Image = new UIImage(asset.Asset.Thumbnail);
								    var representation = asset.Asset.DefaultRepresentation;
									UIImage imgPhoto = new UIImage(representation.GetImage(), representation.Scale, (UIImageOrientation)representation.Orientation);
									if (imgPhoto != null)
									{
										UIImage latestPhoto = new UIImage();
										latestPhoto = MaxResizeImage(imgPhoto, 75, 75);
										var imageView = ImageViewArray[i];
										imageView.Image = latestPhoto;
									}
								}
								else {
									var representation = asset.Asset.DefaultRepresentation;
									...
									if (imgPhoto != null)
									{
										UIImage latestPhoto = new UIImage();
										latestPhoto = MaxResizeImage(imgPhoto, 75, 75);
										var imageView = new UIImageView(latestPhoto);
										ImageViewArray.Add(imageView);
									}
								}
							}
						...
						{
							Console.WriteLine("Failed to set thumbnail " +  e);
						}
